/* Tarmroom - controls devices in the Tarm Room
   Mudbus TCP/IP modbus interface in conjuntion with python under Windows 
   program
   A lot of the code is leftover garbage from the thermostat control program
   V1.0   3/18/16
*/
...
// Data wire for sensors is plugged into Digital pin 15 on the Arduino

DallasTemperature sensors(&oneWire);
DeviceAddress Thermometer0 = {0x28, 0xFF, 0x56, 0x5C, 0x54, 0x14, 0x00, 0xB9};
DeviceAddress Thermometer1 = {0x28, 0x9A, 0xFC, 0x2D, 0x07, 0x00, 0x00, 0x70};
//DeviceAddress Thermometer2 = {0x28, 0xFF, 0x80, 0x55, 0x54, 0x14, 0x00, 0xE8};
DeviceAddress* Thermosensor[2];
...
void setup(void){
 //setup ethernet and start
  uint8_t mac[]   = { 0x90, 0xA2, 0xDA, 0x00, 0x51, 0x20 };   // not the real address
  uint8_t ip[]	= { 192, 168, 0, 52};     // production
//  uint8_t ip[]	= { 192, 168, 10, 152};           // test on desk
  uint8_t gateway[] = { 192,168,0,1};	          // production
...
void loop(void){
	...
	//Add temperature here...
        ...
        for (i=0; i<2; i++) {                        // *** only sets first 2 zones 
           ...
           tempF = DallasTemperature::toFahrenheit(tempC) * 10;   //  for the 10ths 
	   Mb.R[i+3] = (uint16_t) tempF;
        ...
        wdt_reset();
   /*  commented out and awaiting control relays 
